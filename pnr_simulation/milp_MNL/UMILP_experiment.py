...
for (numAlternatives,numOrigins) in [(30,100),(60,200),(90,300),(120,400)]:
    ...
    for instance in range(numInstances):
        ...
        print('### solve opt_unconstrained_pnr_origin%s_I%s_J%s_p%s_inst%s.csv'%(numOrigins,numOrigins*numDestins,numAlternatives,numHubs,instance))
        ...
        # ILP Model
        model = Model('pnr')
        setParam('LogFile', 'grblog_unconstrainedMNL_pnr_origin%s_I%s_J%s_p%s_inst%s_machine%s.txt'%(numOrigins,numOrigins*numDestins,numAlternatives,numHubs,instance,machineName))
        ...
        ilpTable = pd.DataFrame(list(zip(machineArray,originArray,iArray,jArray,pArray,instanceArray,ilpObjArray,accurateArray,constrainedArray,underArray,overArray,runArray)),columns =['Machine','Origins','I','J','p','instance','ILP OPT','Accuarate','ifConstrained','underCapacity','overCapacity','runTime'])
        ilpTable.to_csv(r'ilpTable_unconstrained_pnr_origin%s_I%s_J%s_p%s.csv'%(numOrigins,numOrigins*numDestins,numAlternatives,numHubs), index = False)#Check
