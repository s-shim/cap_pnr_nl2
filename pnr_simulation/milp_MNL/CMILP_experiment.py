...
for (numAlternatives,numOrigins) in [(30,100),(60,200),(90,300),(120,400)]:
    ...
    for instance in range(numInstances):
        ...
        # ILP Model
        model = Model('pnr')
        setParam('LogFile', 'grblog_constrainedMNL_pnr_origin%s_I%s_J%s_p%s_inst%s_machine%s.txt'%(numOrigins,numOrigins*numDestins,numAlternatives,numHubs,instance,machineName))
        ...
        ilpTable.to_csv(r'ilpTable_constrained_pnr_origin%s_I%s_J%s_p%s.csv'%(numOrigins,numOrigins*numDestins,numAlternatives,numHubs), index = False)#Check
